...
const Home = () => {
	...
	const getLatest = async() => {
		const res = await fetch(`https://api.themoviedb.org/3/movie/top_rated?api_key=${process.env.REACT_APP_API_URL}`);
	...
	const getPopular = async() => {
		const res = await fetch(`https://api.themoviedb.org/3/trending/movie/week?api_key=${process.env.REACT_APP_API_URL}`);
		const data = await res.json();
  ...
  return (
   <div>
		...
		<div className='flex flex-col'>
			...
			{currentlyWatchingList.map((item) => (
				<Link key={item.id} to={`/movie/${item.id}`}>
					<img className='w-[177px] h-[263px] rounded-[1.5rem]' src={`https://image.tmdb.org/t/p/original/${item.poster_path}`} alt={item.original_title} />
				</Link>
			))}
			</div>
		</div>
	...
	<div className='px-20 py-10'>
		...
		<div className='flex gap-4 flex-nowrap overflow-x-scroll no-scrollbar' ref={ref} {...events}>
			{popularMovies.map((item) => (
				<Link key={item.id} to={`/movie/${item.id}`}>
					<img className='w-[177px] max-w-max h-[263px] rounded-[1.5rem]' src={`https://image.tmdb.org/t/p/original/${item.poster_path}`} alt="" />
				</Link>
			))}
			</div>
	</div>
